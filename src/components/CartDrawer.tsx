        ...
        <div className="relative flex h-full w-full max-w-md flex-col bg-background shadow-2xl animate-in slide-in-from-right duration-300">
          ...
          <div className="flex items-center justify-between border-b border-border bg-muted/40 px-6 py-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-extrabold">{t("cart")}</h3>
              <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-bold text-primary">
                {cartRecipes.length} {t("cart_items")}
              </span>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {cartRecipes.length === 0 ? (
              <div className="flex h-64 flex-col items-center justify-center text-center">
                ...
                <h4 className="mt-4 text-base font-bold">{t("cart_empty")}</h4>
                <p className="mt-1 text-xs text-muted-foreground">{t("cart_empty_hint")}</p>
              </div>
            ) : (
          ...
          {cartRecipes.length > 0 && (
            <div className="border-t border-border bg-muted/40 p-6 space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground font-bold">{t("total")}</span>
              ...
              >
                {t("order_now")} <ArrowRight className="size-4" />
              </button>
            </div>
          )}
        </div>
