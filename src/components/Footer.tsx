...
export function Footer() {
  ...
  const columns = [
    {
      titleKey: "footer_recipes" as const,
      items: ["Recipes", "Winter salads", "Organic chicken", "Beef and Mutton", "Flavoured Milk"],
    },
    {
      titleKey: "footer_news" as const,
      items: ["Our Blogs", "Contests/Sweepstakes", "Video", "New Releases", "Newsletters"],
    },
    {
      titleKey: "footer_about" as const,
      items: ["FAQ", "Our Board", "Our Staff", "Contact Us"],
    },
  ];
    ...
    <footer className="mt-16 rounded-t-3xl bg-ink text-ink-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-12 md:grid-cols-[1fr_2fr_1.2fr]">
        <div>
          ...
          <p className="mt-4 text-sm opacity-70">{t("all_rights")}</p>
        </div>

        <div className="grid gap-8 sm:grid-cols-3">
          {columns.map((col) => (
            <div key={col.titleKey}>
              <h3 className="text-lg font-bold">{t(col.titleKey)}</h3>
        ...
        <div>
          <h3 className="script-title text-3xl text-ink-foreground">{t("newsletter")}</h3>
          <form
