...
import { emptyFilters, filterRecipes, type FilterState } from "@/lib/recipes";
...
function Explore() {
  ...
  // Sidebar-level toggles (diet, allergies, goals etc.)
  const { filters, toggle, clear } = useFilters();

  // Always override Cusine and Diet from URL so category clicks always work
  const activeFilters: FilterState = {
    ...filters,
    Cusine: cusine ? [cusine] : filters.Cusine,
    Diet: diet ? [diet] : filters.Diet,
  };

  const [query, setQuery] = useState(q ?? "");

  let results = filterRecipes(activeFilters, query);
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-10 px-6 py-10 lg:flex-row">
      <FilterSidebar filters={activeFilters} onToggle={toggle} onClear={clear} />
