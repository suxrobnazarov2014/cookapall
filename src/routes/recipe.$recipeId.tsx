...
export const Route = createFileRoute("/recipe/$recipeId")({
  loader: ({ params }) => {
    ...
    return { recipe: recipe ?? null, recipeId: params.recipeId };
  },
  head: ({ loaderData }) => {
    if (!loaderData?.recipe) {
      return {
        meta: [{ title: "Recipe — Cookpal" }, { name: "robots", content: "noindex" }],
      };
    }
...
function RecipeDetail() {
  const { recipe: loaderRecipe, recipeId } = Route.useLoaderData() as {
    recipe: Recipe | null;
    recipeId: string;
  };
  const { favorites, toggleFavorite, addToCart, customRecipes } = useStore();
  const recipe =
    loaderRecipe ||
    (customRecipes.find((r) => r.id === recipeId) as Recipe | undefined) ||
    getRecipe(recipeId);
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {recipe.hashtags.map((tag) => (
                <Link
                  ...
                  to="/hashtags"
                  search={{ tag }}
                  className="rounded-full bg-primary/10 px-3 py-1 text-xs font-bold text-primary hover:bg-primary hover:text-primary-foreground transition-all"
                >
                  {tag}
                </Link>
              ))}
